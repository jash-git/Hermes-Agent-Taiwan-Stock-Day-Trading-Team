...
# Apply sign from 漲跌(+/-)
sign_col = '漲跌(+/-)'
df['signed_chg'] = df.apply(lambda r: r['漲跌價差'] if r[sign_col] != '-' else -r['漲跌價差'], axis=1)
df['signed_pct'] = df.apply(lambda r: r['chg_pct'] if r[sign_col] != '-' else -r['chg_pct'], axis=1)

# Filter only regular stocks (4-digit codes)
stocks_only = df[df['證券代號'].str.match(r'^\d{4}$')]

print('=== STOCK LOSERS TOP10 ===')
for _, r in stocks_only.nsmallest(10, 'signed_pct').iterrows():
    print(f"{r['證券代號']} {r['證券名稱']} close={r['收盤價']} chg={r['signed_pct']}%")
